...
//Camera variables
float angle = 0.0f;
float lookx = 0.0f, looky = 124.82f, lookz = -1.0f;
//float camPosx = 0.0f, camPosy = 1.0f, camPosz = 5.0f; 
//float lookx = 0.274823606f, looky = 105.980118f, lookz = 0.961494684f;
float camPosx = -516.8f, camPosy = 125.0f, camPosz = -1642.55f;
...
glm::vec3 g_cameraXYZ = glm::vec3(-516.8f, 125.0f, -1642.55f);
glm::vec3 g_cameraTarget_XYZ = glm::vec3(-516.8f, 124.82f, -1647.55f);
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	{
	case GLFW_KEY_P:
		g_cameraTarget_XYZ = g_vecGameObject[shipIndex++/2]->position; //Doubling all my numbers because key release triggers a second key press.
		if (shipIndex == 74)
		{
			shipIndex = 0;
		}
